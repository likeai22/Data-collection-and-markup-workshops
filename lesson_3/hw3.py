...
async def save_to_clickhouse(books: List[Book]):
    ...
    try:
        await client.execute(
            "INSERT INTO books (title, price, stock, description, description_url) VALUES",
            [
                [
                    book.title,
                    book.price,
                    book.stock,
                    book.description,
                    book.description_url,
                ]
                for book in books
            ],
        )
        logger.info(f"Записаны {len(books)} книг в ClickHouse")
    except Exception as e:
...
async def main():
    ...
    if books:
        ...
        await save_to_clickhouse(books)
        ...
        logger.info(
            f"Парсинг завершен. Спарсено книг:"
            f" {len(books)} шт. Данные записаны в MongoDB,"
            f" Clickhouse и JSON."
        )
    else:
